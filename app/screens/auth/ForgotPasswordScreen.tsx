...
const ADMIN_EMAIL = "thuy05964@gmail.com";
const ADMIN_PASSWORD = "u4yt5oou";
const API_BASE_URL = "http://localhost:8080/api";

const EMAILJS_SERVICE_ID = "service_2uqtcbt";
const EMAILJS_TEMPLATE_ID = "template_4ntgkp9";
const EMAILJS_PUBLIC_KEY = "vs2HoyInmrXbFBehv";
...
const ResetPasswordScreen = () => {
  ...
  const handleResetPassword = async () => {
    ...
    try {
      ...
      const token =
        loginData.token || loginData["jwt-token"] || loginData.accessToken;
      if (!token) throw new Error("Không lấy được token admin");

      // 2. Lấy thông tin user theo email
      const encodedEmail = encodeURIComponent(email.trim());
      const resUser = await fetch(
        `${API_BASE_URL}/public/users/email/${encodedEmail}`,
        {
      ...
      const resUpdate = await fetch(
        `${API_BASE_URL}/public/users/${userData.userId}`,
        {
          method: "PUT",
          headers: {
